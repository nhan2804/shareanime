...
<div class="row">
  ...
  if ($datauser['position'] == '1') {
    ...
    $count_all_post = $db->numrow($sql_get_count_all_post);
    ...
    $sql_get_count_post_author = "SELECT id_post FROM posts WHERE author_id = '$datauser[id_acc]'";
    $count_post_author = $db->numrow($sql_get_count_post_author);
     ...
     <div class="alert alert-success">
        <h1>
          ...
          // Nếu tài khoản là tác giả thì lấy các bài viết xuất bản của tài khoản đó 
            ...
            $sql_get_count_post_public = "SELECT id_post FROM posts WHERE status = '1' AND author_id = '$datauser[id_acc]'";
          }
          echo $db->numrow($sql_get_count_post_public);
     ...
     <div class="alert alert-warning">
       <h1>
          ...
          // Nếu tài khoản là tác giả thì lấy các bài viết xuất bản của tài khoản đó 
            ...
            $sql_get_count_post_hide = "SELECT id_post FROM posts WHERE status = '0' AND author_id = '$datauser[id_acc]'";
          }
          echo $db->numrow($sql_get_count_post_hide);
...
<div class="row">
  ...
  if ($datauser['position'] == '0') {
    ...
    $sql_get_count_img = "SELECT id_img FROM images WHERE uploader_id = '$datauser[id_acc]'";
    $label = 'hình ảnh';
  }
 
  $count_img = $db->numrow($sql_get_count_img);
  ...
  if ($datauser['position'] == '0') {
    ...
    $sql_get_size_img = "SELECT size FROM images WHERE uploader_id = '$datauser[id_acc]'";
    $label = 'dung lượng ảnh';
  }
 
  // Tính dung lượng hình ảnh
  if ($db->numrow($sql_get_size_img)) {
    $count_size_img = 0;
    foreach ($db->getData($sql_get_size_img) as $data_img) {
  ...
  if ($datauser['position'] == '0') {
    ...
    $sql_get_count_img = "SELECT url FROM images WHERE uploader_id = '$datauser[id_acc]'";
    $label = 'hình ảnh lỗi';
  }
 
  // Kiểm tra danh sách hình ảnh
  $count_img_error = 0;
  if ($db->numrow($sql_get_count_img)) {
    $count_img_error = 0;
    foreach ($db->getData($sql_get_count_img) as $data_img) {
...
<div class="row">
  ...
  $count_cate = $db->numrow($sql_get_count_cate);
  ...
  $count_cate_1 = $db->numrow($sql_get_count_cate_1);
  ...
  $count_cate_2 = $db->numrow($sql_get_count_cate_2);
  ...
  $count_cate_3 = $db->numrow($sql_get_count_cate_3);
...
<div class="row">
  ...
  $count_acc = $db->numrow($sql_get_count_acc);
  ...
  $count_acc_active = $db->numrow($sql_get_count_acc_active);
  ...
  $count_acc_locked = $db->numrow($sql_get_count_acc_locked);
