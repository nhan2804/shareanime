...
if ($user) 
{
    ...
    if (isset($_FILES['img_avt'])) 
    {
        ...
        $sql_up_avt = "UPDATE accounts SET url_avatar = '$url_img' WHERE id_acc = '$datauser[id_acc]'";
        $db->statement($sql_up_avt);
    ...
    else if (isset($_POST['action']))
    {
        ...
        {       
            if (file_exists('../'.$datauser['url_avatar']))
            {
                unlink('../'.$datauser['url_avatar']);
            }
 
            $sql_delete_avt = "UPDATE accounts SET url_avatar = '' WHERE id_acc = '$datauser[id_acc]'";
            $db->statement($sql_delete_avt);
            $db->close();    
    ...
    if ($dn_update && $email_update) {
        ...
        // Kiểm tra số điện thoại
            ...
            $sql_update_info = "UPDATE accounts SET
                ...
                WHERE id_acc = '$datauser[id_acc]'
            ";
            $db->statement($sql_update_info);
